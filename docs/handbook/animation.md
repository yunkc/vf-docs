...
- 如果不设置fps， 默认的fps是30， 它表示一秒钟是30帧， 一帧相当于 1/30 = 16.667（毫秒）
:::
## 数据定义
- 动画数据的定义：
``` typescript
...
- 如果包含多个动画的话，想要播放不同的动画，可是使用[动作脚本](action.md)的gotoPlay语句进行控制。
...
- 在一个自定义组件的动画中，可以给事件添加一个时间轴，用来在当动画播放到某一帧时抛出事件，再由[VFX](action.md)监听事件后执行一定的逻辑。
